...
class ScaleAndFadeTransformer extends PageTransformer {
  ...
  ScaleAndFadeTransformer({double fade = 0.8, double scale = 0.0})
      : _fade = fade,
        _scale = scale;
...
class FadeTransformer extends PageTransformer {
  @override
  Widget transform(Widget child, TransformInfo info) {
    double? position = info.position;
    if (position! <= 0.0) {
...
class FadeInTransformer extends PageTransformer {
  @override
  Widget transform(Widget child, TransformInfo info) {
    double? position = info.position;
    if (position! < 0.0) {
      // Page is scrolling to the left
      return Opacity(
        opacity: 1.0,
        child: child,
      );
      ...
      // Page is scrolling to the right
      return Opacity(
        opacity: 1.0,
        child: child,
      );
